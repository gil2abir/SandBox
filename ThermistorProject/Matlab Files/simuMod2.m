...
tau=20 ;  %time contant of thermistor , [sec]
...
T_NoCon1=a1*(1-exp(-m1*t))+a2*(1-exp(-m2*t))+T0;
...
for j=1:length(Imax);
    ...
    for n=3:lt
        ...
        Imod1(n)=ThermCurrent(Tmod1(n),Tmod1(n-1),Rth(n),Ct*0.315,Imax(j),dt);
        V(n)=I(n)*Rth(n);
%         if flag==1;
%           if T(n)>Ttau
%             flag=0;
